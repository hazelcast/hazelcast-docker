...
function assert_augment_with_classifier_tags {
  local INITIAL_TAGS=($1)
  local CLASSIFIER=$2
  local CURRENT_JDK=$3
  local DEFAULT_JDK=$4
  local EXPECTED_TAGS_TO_PUSH=$5
  local ACTUAL_TAGS_TO_PUSH=$(augment_with_classifier_tags "${INITIAL_TAGS[*]}" "$CLASSIFIER" "$CURRENT_JDK" "$DEFAULT_JDK")
  local MSG="Suffixed tags to push for (tags=$INITIAL_TAGS classifier=$CLASSIFIER current_jdk=$CURRENT_JDK default_jdk=$DEFAULT_JDK) should be equal to: $EXPECTED_TAGS_TO_PUSH"
  assert_eq "$EXPECTED_TAGS_TO_PUSH" "$ACTUAL_TAGS_TO_PUSH" "$MSG" && log_success "$MSG" || TESTS_RESULT=$?
}
...
function assert_get_tags_to_push {
  ...
  local ACTUAL_TAGS_TO_PUSH=$(get_tags_to_push "$VERSION_TO_RELEASE" "$CLASSIFIER" "$CURRENT_JDK" "$DEFAULT_JDK" "$IS_LATEST_LTS")
  local MSG="Tags to push for (version_to_release=$VERSION_TO_RELEASE classifier=$CLASSIFIER current_jdk=$CURRENT_JDK default_jdk=$DEFAULT_JDK is_latest_lts=$IS_LATEST_LTS) should be equal to: $EXPECTED_TAGS_TO_PUSH "
  assert_eq "$EXPECTED_TAGS_TO_PUSH" "$ACTUAL_TAGS_TO_PUSH" "$MSG" && log_success "$MSG" || TESTS_RESULT=$?
}
...
log_header "Tests for augment_with_classifier_tags"
assert_augment_with_classifier_tags "1.2.3" "" "11" "11" "1.2.3-jdk11 1.2.3"
assert_augment_with_classifier_tags "1.2.3 latest" "" "11" "11" "1.2.3-jdk11 1.2.3 latest-jdk11 latest"
assert_augment_with_classifier_tags "1.2.3" "" "17" "11" "1.2.3-jdk17"
assert_augment_with_classifier_tags "1.2.3 latest" "" "17" "11" "1.2.3-jdk17 latest-jdk17"
assert_augment_with_classifier_tags "1.2.3" "slim" "11" "11" "1.2.3-slim-jdk11 1.2.3-slim"
assert_augment_with_classifier_tags "1.2.3" "slim" "17" "11" "1.2.3-slim-jdk17"
assert_augment_with_classifier_tags "1.2.3 latest" "slim" "17" "11" "1.2.3-slim-jdk17 latest-slim-jdk17"
assert_augment_with_classifier_tags "1.2.3 latest-lts" "slim" "17" "11" "1.2.3-slim-jdk17 latest-lts-slim-jdk17"
tags_array=(1.2.3 latest)
assert_augment_with_classifier_tags "${tags_array[*]}" "slim" "17" "11" "1.2.3-slim-jdk17 latest-slim-jdk17"
assert_augment_with_classifier_tags "1.2.3 1.2" "slim" "17" "11" "1.2.3-slim-jdk17 1.2-slim-jdk17"
